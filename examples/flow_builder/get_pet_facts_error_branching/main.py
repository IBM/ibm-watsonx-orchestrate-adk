...
from examples.flow_builder.get_pet_facts_error_branching.tools.get_pet_facts_error_branching import build_get_pet_facts_error_branching_flow
...
async def main():
    ...
    my_flow_definition = await build_get_pet_facts_error_branching_flow().compile_deploy()
    global flow_name
    flow_name = my_flow_definition.flow.spec.display_name
    generated_folder = f"{Path(__file__).resolve().parent}/generated"
    my_flow_definition.dump_spec(f"{generated_folder}/get_pet_facts_error_branching.json")
