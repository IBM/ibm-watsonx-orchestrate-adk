...
class CurrencyAgent:
    ...
    def __init__(self):
        api_key = os.getenv('WATSONX_APIKEY', 'EMPTY')
        llm_url = os.getenv('WATSONX_URL', 'https://us-south.ml.cloud.ibm.com')
        space_id = os.getenv('WATSONX_SPACE_ID', 'EMPTY')
        
        self.model = ChatWatsonx(
            model_id=os.getenv('TOOL_LLM_NAME', 'meta-llama/llama-3-2-90b-vision-instruct'),
            apikey=SecretStr(api_key),
            url=SecretStr(llm_url),
            space_id=space_id,
            temperature=0
        )
