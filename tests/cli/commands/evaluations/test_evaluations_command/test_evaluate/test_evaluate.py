...
class TestLegacyEvalFlag:
    ...
    def test_evaluate_with_legacy_eval_false(self, config_file, user_env_file, monkeypatch):
        ...
        # Set USE_LEGACY_EVAL to FALSE
        monkeypatch.setenv("USE_LEGACY_EVAL", "FALSE")
        
        # Mock Config and agentops to prevent API key prompts - these stay active throughout
        with patch("ibm_watsonx_orchestrate.cli.commands.evaluations.evaluations_controller.Config"), \
             patch("agentops.main.main") as mock_agentops_main:
            # Reload modules to pick up the new environment variable
            import importlib
            from ibm_watsonx_orchestrate.cli.commands.evaluations import evaluations_controller, evaluations_command as eval_cmd
            importlib.reload(evaluations_controller)
            importlib.reload(eval_cmd)  # Reload eval_cmd so it gets the reloaded controller
            
            # Apply mock AFTER reload so it applies to the reloaded module
            with patch.object(
                evaluations_controller.EvaluationsController, "evaluate"
            ) as mock_evaluate:
                eval_cmd.evaluate(
                    config_file=config_file, user_env_file=user_env_file
                )
        ...
        with patch("ibm_watsonx_orchestrate.cli.commands.evaluations.evaluations_controller.Config"), \
            ...
            with patch.object(
                ...
                with caplog.at_level(logging.WARNING):
                    ...
                    # Check that the beta warning messages are present
        ...
        with patch("ibm_watsonx_orchestrate.cli.commands.evaluations.evaluations_controller.Config"), \
            ...
            with patch.object(
                ...
                with caplog.at_level(logging.WARNING):
                    ...
                    # Check that the beta warning messages are NOT present
...
"""
    ...
    def test_evaluate_with_config_option(self, config_file, user_env_file, monkeypatch):
    ...
    def test_evaluate_with_test_paths_option(self, user_env_file, monkeypatch):
    ...
    def test_evaluate_with_output_dir_option(self, user_env_file, monkeypatch):
    ...
    def test_evaluate_with_env_file_option(self, monkeypatch):
...
WATSONX_APIKEY=custom_key"""
    ...
    def test_evaluate_with_env_manager_path_option(self, env_manager_file, user_env_file, monkeypatch):
        ...
        with patch("ibm_watsonx_orchestrate.cli.commands.evaluations.evaluations_controller.Config"), \
            ...
            # Reload in correct order: controller first, then env_manager, then command (which imports both)
            importlib.reload(evaluations_controller)
            importlib.reload(evaluations_environment_manager)
            importlib.reload(eval_cmd)  # Reload eval_cmd LAST so it gets both reloaded modules

            # Mock the function in eval_cmd module since that's where it's called from (line 189)
            with patch.object(
                eval_cmd, "run_environment_manager"
            ) as mock_env_manager:
                output_dir = "test_output"
                eval_cmd.evaluate(
                    env_manager_path=env_manager_file,
                    output_dir=output_dir,
                    user_env_file=user_env_file
                )
    ...
    def test_evaluate_with_langfuse_option(self, user_env_file, monkeypatch):
    ...
    def test_evaluate_all_options_combined(self, config_file, user_env_file, monkeypatch):
